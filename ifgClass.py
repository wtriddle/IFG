# Written by William Riddle from 9/23/19 - 3/10/20
...
class ifg:
    ...
    def __init__(self, smilescode):
        ...
        # Accessible data to be retrieved from algorithm
        self.SMILES = formatSMILEScode(smilescode)
        self.CYCLICINDICES = initializeCYCLICINDICES(SMILES)
        self.RINGPOSITIONS = initializeRINGPOSITIONS(SMILES)
        self.ALCOHOLICINDICES = []
        self.AROMATICINDICIES = initializeAROMATICINDICES(SMILES)
        self.FGDATA = functionalGroups(SMILES)
    ...
    def checkGroup(group, template, full=True):
    	...
    	if difference == 0:
    		...
    		for symbol in template:
    			...
    			if groupSymbol not in ATOMS and symbol == 'R': # If symbol is an R, and the groupSymbol is not an atom at the required position, then the group match is false
    				return False
    ...
    def formatSMILEScode(SMILEScode):
    	...
    	while SMILEScodePos != SMILEScodelength:
    		SMILEScodePos += 1
            symbol = SMILEScode[SMILEScodePos]
    		if symbol == '[':
