...
def initializeRINGPOSITIONS():
	...
	for symbol in SMILEScode:
		...
		if symbol in ATOMS:
			atomIndex += 1
			# Atom found before number becomes opening atom because subLoop is ran right after
			openingAtom = symbol
		...
		if symbol in NUMBERS and smilesPos not in evaluatedNumbers:
			...
			# Special case where charged atom is opening the ring, a closing bracket precedes number. Re-assign positional variables
			if SMILEScode[smilesPos-1] == ']': 
				specialCounter = smilesPos - 1 
...
# Evaluates the FGdata global and determines the valid groups within the strucutre, with proper occurances
...
# 1. compareGroup is fully contained within group
#
		...
		for compareGroup in FGdata:
			...
			for atom in groupDict:
				for compareAtom in compareDict:
					if atom[0] == compareAtom[0] and atom[0] != 'R': # If the symbols are maingroup equivalent
						if atom[1] == compareAtom[1]: # If the atoms are the same index
							mainGroupCrossover += 1 # Then atoms must be of the same maingroup
			
			# Check if compareGroup is fully contained within group (B.1) Skip if false
			if mainGroupCrossover == numMainCompareAtoms and len(groupIndicies) > len(compareIndicies):
				continue

			# Check if group is fully contained within compareGroup (C.1) Skip if false
...
# Called from evaluateFGdata, checks if a group is already contained within the list. Only EXACT matches, i.e fginfo === fginfo for all cells is true
def isGroupInList(formattedFGdata, group):
	...
	if formattedFGdata:
		...
		for compareGroup in formattedFGdata:
			...
			numMainCompare = len(re.compile(r'R').findall(compareGroupTemplate)) - len(compareGroupTemplate)
				...
				if len(compareGroupIndicies) == len(groupIndicies):
					...
					for atom in groupDict:
						for compareAtom in compareGroupDict:
							...
							# Check for equivalent maingroup atoms, in same positional index
							if atom[0] == compareAtom[0] and atom[0] != 'R':
								if atom[1] == compareAtom[1]:
									mainGroupCrossover += 1
									
					# If there is full maingroups crossover and the templates are equal
					if mainGroupCrossover == numMainCompare == numMainGroupAtoms and compareGroupTemplate == groupTemplate:
...
def atomHandler(atomSymbol, atomPosition, atomIndex):	
	...
	if atomCount > 2:
		...
		for symbol in rInnerParenthGroup:
			...
			# Break at first atom so that expandGroup can finish the
			if symbol in ATOMS: 
				break
		...
		# endPosition is final position of rInnerParenthGroup added to 1 to compensate for final ')'
...
def whichGroup(startPosition, endPosition, group, atomIndicies):
	...
	for line in f.open('FGlist.txt', 'r'):
		...
		# Unequivalent lengths with a true match means a portion match
			...
			portionMatches.append(FGtemplate)
...
# Expands a group passed into whichGroup if it is found to be a portion of another larger group. Returns fully matched and expanded group info, or False if nothing was matched
